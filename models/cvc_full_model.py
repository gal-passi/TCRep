...
class CVCModel(nn.Module):
    ...
    def get_transformer_embeddings(
        ...
        Returns:
        - NumPy array of shape (num_seqs, hidden_dim * len(layers)) for aggregated methods.
        - NumPy array of shape (num_seqs, seq_len, hidden_dim * len(layers)) for "full" method.
        """
        ...
        for seq_chunk in tqdm(chunks_zipped, disable=not pbar):
            ...
            for i in range(len(seq_chunk[0])):
                ...
                if method == "full":
                    # For full embeddings, concatenate layers along the last dimension
                    e = torch.cat(e, dim=-1)  # Shape: (seq_len, hidden_dim * len(layers))
                else:
                    e = torch.cat(e)
...
class CVCClassifierModelFullEmbed(nn.Module):
    def __init__(self, model_dir: str = TRANSFORMER, method: str = 'full', ch_dropout: float = 0.0, device: str = 'cuda',
                 batch_size: int = 256, freeze_embed_model: bool = False,
                  cvc_layers_to_train: int = 3, lora: bool = False,
        ...
        self.max_seq_length = max_seq_length  # Default maximum sequence length for padding/truncation

        # Determine input dimension based on method
        if method == 'full':
            # For full embeddings: max_seq_length * hidden_dim
            input_dim = self.max_seq_length * 768
        else:
            # For aggregated embeddings: hidden_dim
            input_dim = 768
        ...
        if ch_type == 'none':
            ...
            if method == 'full':
                # For full embeddings, we need to handle the 2D structure differently
                self.linear = nn.Sequential(
                    nn.Unflatten(1, (self.max_seq_length, 768)),  # reshape to (batch_size, seq_len, 768)
                    nn.Flatten(start_dim=2),  # flatten last dimension if needed
                    nn.Conv1d(self.max_seq_length, 32, kernel_size=3, padding=1),  # treat seq_len as channels
                    nn.ReLU(),
    ...
    def _pad_and_truncate_embeddings(self, embeddings):
        """
        Pad or truncate embeddings to a fixed length and flatten them.

        Args:
            embeddings: Tensor of shape (batch_size, seq_len, hidden_dim) for full embeddings

        Returns:
            Tensor of shape (batch_size, max_seq_length * hidden_dim)
        """
        ...
        if seq_len > self.max_seq_length:
            ...
            # Pad with zeros
            padding = torch.zeros(batch_size, self.max_seq_length - seq_len, hidden_dim,
                                device=embeddings.device, dtype=embeddings.dtype)
            embeddings = torch.cat([embeddings, padding], dim=1)

        # Flatten the sequence dimension
        embeddings = embeddings.view(batch_size, -1)  # Shape: (batch_size, max_seq_length * hidden_dim)
